...
std::vector<struct Complex> initialize_matrix(int W, int H) {
    static std::vector<struct Complex> mat(H * W);
...
std::vector<int> computeMandelbrotMat(std::vector<struct Complex> mat, int maxIter) {
    auto begin = mat.begin();
    auto end = mat.end();

    std::vector<int> res(mat.size());
    auto it_res =  res.begin();

    for (; begin != end; begin ++) {
        int iter = iterMandelbrot(maxIter, *begin);
        *it_res = iter;
        it_res++;
    }
    return res;
...
int main(int argc, char *argv[] )
{
    ...
    int count = 2;
    const int block_length[count] = {1, 1};
    MPI_Datatype types[count] = {MPI_DOUBLE, MPI_DOUBLE};
    MPI_Aint displacements[count] = {offsetof(Complex, real), offsetof(Complex, imag)};
    MPI_Datatype MPI_Complex;
    MPI_Type_create_struct(count, block_length, displacements, types, &MPI_Complex);

    if ((W * H) % nbp != 0) {
        printf("The number of processes must divide the size of the image !\n");
        return EXIT_FAILURE;
    }

    std::vector<struct Complex> recv_buf((W * H) / nbp);
    if (rank == 0) {
        std::vector<struct Complex> mat = initialize_matrix(W, H);

        MPI_Scatter(&mat, W * H, MPI_Complex, &recv_buf, W * H, MPI_Complex, 0, globComm);
    }
