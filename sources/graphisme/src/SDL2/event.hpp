...
class event {
  public:
    ...
    virtual std::int32_t kind_of_event() const { return none; }
};
// =============================================================================================
class event_quit : public event {
  public:
    ...
    virtual std::int32_t kind_of_event() const override { return event::quit; }
};
// =============================================================================================
class event_mouse_motion : public event {
  public:
    event_mouse_motion(SDL_MouseMotionEvent *event)
        : position_x(event->x), position_y(event->y), relative_x(event->xrel),
...
class event_mouse_button : public event {
  public:
    ...
    event_mouse_button(SDL_MouseButtonEvent *event)
        : _id_button(event->button), _state_of_button(event->state),
          _number_of_clicks(event->clicks), _position_x(event->x),
          _position_y(event->y) {}
    ...
    std::int32_t button() const { return this->_id_button; }

    std::int8_t state() const { return this->_state_of_button; }

    std::int8_t number_of_clicks() const { return this->_number_of_clicks; }
  ...
  private:
    ...
    std::uint8_t _number_of_clicks; // Nombre de cliques (double clique, triple
                                    // clique, etc.)
    std::int32_t _position_x,
        _position_y; // Position de la souris au moment de l'événement
};
// =============================================================================================
class event_keyboard : public event {
  public:
    ...
    event_keyboard(SDL_KeyboardEvent *event)
        : _state(event->state), _code(event->keysym.sym),
          _repeat(event->repeat) {}

    virtual std::int32_t kind_of_event() const override { return event::key; }
    ...
    std::int32_t code() const { return this->_code; }

    std::uint8_t state() const { return this->_state; }

    std::uint8_t repeat() const { return this->_repeat; }

  private:
    std::uint8_t _state;
    std::int32_t _code;
    std::uint8_t
        _repeat; // Nombre de fois que la touche a été enfoncé rapidement
};
// ---------------------------------------------------------------------------------------------
class event_touch : public event {
  public:
    event_touch(SDL_TouchFingerEvent *evt)
        : x(evt->x), y(evt->y), dx(evt->dx), dy(evt->dy),
          pressure(evt->pressure), type(evt->type) {}

    virtual std::int32_t kind_of_event() const override {
        return event::touch_finger;
    }

    std::int32_t type_of_event() const { return this->type; }
    ...
    std::array<float, 2> displacement() const {
        return std::array<float, 2>({this->dx, this->dy});
    }
  ...
  private:
    ...
    float
        pressure; // La quantité de pression exercée par le doigt (entre 0 et 1)
    std::int32_t type;
};
// =============================================================================================
class event_queue {
