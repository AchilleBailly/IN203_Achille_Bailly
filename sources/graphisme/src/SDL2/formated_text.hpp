...
class formated_text : public image {
  public:
    ...
    enum numbering { latin = 0, roman, alpha };
    ...
    class font_manager_t {
      public:
        ...
        iterator load_and_add(const std::string &key, const std::string &fonte,
                              std::uint8_t size);
    ...
    formated_text(const std::array<std::int32_t, 4> &zone,
                  const std::string &path_for_font);
