    ...
    while (!quitting) {
        ...
        for (const auto &e : events) {
            if (e->kind_of_event() == sdl2::event::quit) {
                quitting = true;
                MPI_Isend(&quitting, 1, MPI_INT, 1, 10, globComm, &req);
            }
        }
        ...
        if (affiche) {
            ...
            std::vector<StatistiqueParCase> tmp(dim_x * dim_y);
...
void simulation_process1(bool affiche) {
    ...
    while (!quitting) {
        ...
        MPI_Ssend(grille.m_statistiques.data(), dim_x * dim_y, MPI_StatParCase,
                  0, 42, globComm);
...
int main(int argc, char *argv[]) {
    ...
    if (nbp != 2) {
        std::cerr << "Choose eexactly 2 processes.\n";
        exit(-1);
    }
    int rank;
    MPI_Comm_rank(globComm, &rank);
