...
épidémie::Individu::Individu(unsigned int graine_aléatoire,
                             int espérance_de_vie, bool estNé,
                             int déplacement_max)
    : m_âge{}, m_espérance_de_vie{espérance_de_vie}, m_position{},
      m_moteur_stochastique{graine_aléatoire},
      m_générateur_quantité_déplacement(1, déplacement_max),
      m_générateur_déplacement(1, 4), m_générateur_maladie(0., 1.),
      m_générateur_âge(1, espérance_de_vie) {
    if (estNé) {
        m_âge = 1;
    } else
        m_âge = m_générateur_âge(m_moteur_stochastique);

    m_grippe.sensibilité = Sensibilité::Sensible;
    m_agent_pathogène.sensibilité = Sensibilité::Sensible;
}
// --------------------------------------------------------------------------------------------------------------------
void épidémie::Individu::veillirDUnJour() {
...
void épidémie::Individu::seDéplace(Grille &grille) {
    enum Direction { Ouest = 1, Est = 2, Nord = 3, Sud = 4 };
    int nbre_déplacement =
        m_générateur_quantité_déplacement(m_moteur_stochastique);
    for (int i = 0; i < nbre_déplacement; ++i) {
        ...
        if (m_position.x < 0)
            m_position.x = largeur - 1;
        if (m_position.x >= largeur)
            m_position.x = 0;
        if (m_position.y < 0)
            m_position.y = hauteur - 1;
        if (m_position.y >= hauteur)
            m_position.y = 0;
    }
}
// --------------------------------------------------------------------------------------------------------------------
void épidémie::Individu::estContaminé(Grippe &grippe) {
    if (m_grippe.sensibilité == Sensibilité::Sensible) {
        m_grippe.temps_incubation =
            grippe.nombreJoursIncubation(m_moteur_stochastique);
        m_grippe.temps_symptomatique =
            grippe.nombreJoursSymptomatique(m_moteur_stochastique);
    }
}
// --------------------------------------------------------------------------------------------------------------------
void épidémie::Individu::estContaminé(AgentPathogène &agent) {
    if (m_agent_pathogène.sensibilité == Sensibilité::Sensible) {
        m_agent_pathogène.temps_asymptomatique =
            agent.nombreJoursAsymptomatique(m_moteur_stochastique);
        m_agent_pathogène.temps_symptomatique =
            agent.nombreJoursSymptomatique(m_moteur_stochastique);
        m_agent_pathogène.temps_passé_immunisé = agent.nombre_jours_immunité;
        m_agent_pathogène.temps_contagieux =
            m_agent_pathogène.temps_asymptomatique +
            m_agent_pathogène.temps_symptomatique +
...
// --------------------------------------------------------------------------------------------------------------------
bool épidémie::Individu::testContaminationGrippe(
    ...
    // : On traite d'abord les contaminants ayant le virus mais pas l'agent
    // pathogène :
    double probabilité_attraper_grippe = t_interactions.tauxTransmissionGrippe(
        grippe.tauxTransmission(), false,
    ...
    for (int i = 0; i < statistiques.nombre_contaminant_seulement_grippé; ++i) {
        if (m_générateur_maladie(m_moteur_stochastique) <
            probabilité_attraper_grippe) {
            return true;
        }
    }
    probabilité_attraper_grippe = t_interactions.tauxTransmissionGrippe(
        grippe.tauxTransmission(), true,
        this->m_agent_pathogène.sensibilité == Sensibilité::Infecté,
        this->m_agent_pathogène.sensibilité == Sensibilité::Immunisé);
    for (int i = 0;
         i < statistiques.nombre_contaminant_grippé_et_contaminé_par_agent;
         ++i) {
        if (m_générateur_maladie(m_moteur_stochastique) <
            probabilité_attraper_grippe) {
            return true;
...
//
bool épidémie::Individu::testContaminationAgent(Grille const &grille,
                                                AgentPathogène &agent) {
    ...
    for (int i = 0;
         i < statistiques.nombre_contaminant_seulement_contaminé_par_agent;
         ++i) {
...
//
void épidémie::Individu::setPosition(int largeur, int hauteur) {
