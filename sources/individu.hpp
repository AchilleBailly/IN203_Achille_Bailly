...
enum struct Sensibilité { Sensible, Contagieux, Infecté, Immunisé };

namespace épidémie {
class Individu {
  public:
    Individu(unsigned int graine_aléatoire, int espérance_de_vie,
             bool estNé = false, int déplacement_max = 10);

    Sensibilité getEtatGrippal() const { return m_grippe.sensibilité; }

    Sensibilité getEtatAgentPathogène() const {
        return m_agent_pathogène.sensibilité;
    }

    bool aGrippeContagieuse() const { return m_grippe.temps_contagieux > 0; }
    ...
    bool doitMourir() const { return m_âge >= m_espérance_de_vie; }

    void seDéplace(Grille &grille);
    void setPosition(int largeur,
                     int hauteur); // Positonne un individu au hasard...
    ...
    bool testContaminationGrippe(Grille const &grille,
                                 Interactions const &t_interactions,
