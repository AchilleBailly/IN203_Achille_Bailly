...
void simulation(bool affiche) {
    ...
    contexte.taux_population = 700'000;
    ...
    while (!quitting) {
        ...
        t3.setEnd();
        // On parcout la population pour voir qui est contaminé et qui ne l'est
        // pas, d'abord pour la grippe puis pour l'agent pathogène
        ...
        for (auto &personne : population) {
            ...
            // veillesse et on génère une nouvelle personne si c'est le cas.
    ...
    std::cout << "Itération totale : " << t0.getAverageDuration().count()
              << "\n";
    std::cout << "Premier jour de l'annee : " << t1.getAverageDuration().count()
              << "\n";
    ...
    std::cout << "Affichage de la grille : " << t5.getAverageDuration().count()
              << "\n";
    output.close();
}
